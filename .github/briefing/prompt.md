...
```json
{
  ...
  "top_caps": {
    "kospi": [
      { "ticker": "373220", "name": "LG에너지솔루션", "comment": "쉬운 말 한 줄 (왜 오르내리는지)" }
    ],
    "kosdaq": [
      { "ticker": "196170", "name": "알테오젠", "comment": "쉬운 말 한 줄" }
    ]
  },
...
### `top_caps` 작성 규칙 (중요)
워크플로우가 백엔드에서 시총 상위 종목과 **지지/저항·종가·등락률을 미리 계산해** 넘겨준다
(`TOP_CAPS_INPUT` 파일). 너는 **그 목록의 종목에 대해 `comment`만** 쓴다.

- `ticker`/`name`은 넘겨받은 값을 **그대로** 복사한다. 순서도 유지한다.
- **종가·등락률·지지·저항 숫자는 절대 쓰지 마라.** 워크플로우가 백엔드 계산값으로 덮어쓴다.
  (지지/저항은 `scipy` 피봇 탐지 알고리즘 결과다 — 네가 추정하면 틀린 숫자가 된다)
- `comment`는 **한 문장, 40자 내외**. 최근 24시간 뉴스·수급 근거로 "왜 그런지"를 쉬운 말로.
  예: `"환율이 높아 수출에 유리하고 외국인이 사들이는 중이에요."`
- 근거를 못 찾은 종목은 `comment`를 생략한다. **지어내지 마라.**
- `TOP_CAPS_INPUT` 파일이 없거나 비어 있으면 `top_caps` 키 자체를 **생략**한다.
