...
def _support_resistance(
    ...
    #
    # ⚠ 같은 봉이 최대 3번 담기는 것은 '중복 버그'가 아니다 — 건드리지 말 것.
    # 20봉 피봇은 정의상 10봉·5봉 피봇이기도 하므로, 넓은 윈도우까지 살아남은
    # 뚜렷한 피봇일수록 여러 번 담긴다. 즉 아래 touch_count는 '터치 횟수'와
    # '피봇 현저성(prominence)'이 섞인 값이고, 그 현저성 신호가 실제로 일을 한다.
    #
    # 인덱스로 중복 제거를 시도했다가 32종목 백테스트에서 품질이 떨어져 되돌렸다:
    #   지지 존중률 58.1% → 55.6% (대조군 대비 -2.9%p → -5.4%p)
    #   저항 존중률 47.6% → 46.6% (대조군 대비 +2.8%p → +1.8%p)
    #   (backtest_results: sr_baseline32 vs sr_dedup32)
    # 개선하려면 현저성을 없앨 게 아니라, 터치 횟수와 분리해 각각 가중치를 주고
    # 백테스트로 튜닝해야 한다.
